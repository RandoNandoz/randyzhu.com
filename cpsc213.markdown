---
layout: collection 
header:
    image: "/assets/glacier-vistas-fullsize.avif"
permalink: /cpsc213
author_profile: true
---

# Slides for CPSC 213

You can find my slides for CPSC 213 2025W2 below.

- Lab 1 --- Intro Lab: [pdf](./cpsc213/L1.pdf){:target="_blank"}
- Lab 2 --- CPU and Static Variables [pdf](./cpsc213/L2.pdf){:target="_blank"}
- Lab 3 --- Dynamic vs Static Arrays [pdf](./cpsc213/L3.pdf){:target="_blank"}
- Lab 4 --- Structs and Dynamic Allocation [pdf](./cpsc213/L4.pdf){:target="_blank"}
- Lab 5 --- Memory Management & Reference Counting [pdf](./cpsc213/L5.pdf){:target="_blank"}, [in class demo](https://github.com/RandoNandoz/213-l5-demos)
- Lab 6 --- Static Control Flow [pdf](./cpsc213/L6.pdf){:target="_blank"}